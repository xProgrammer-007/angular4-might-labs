...
export class MechanicalPostDetailComponent {
  ...
  constructor(private mechanicalSvc: MechanicalService,public routes:ActivatedRoute) {
  this.mechanicals = this.mechanicalSvc.getMechanical(this.routes.snapshot.params['num']);
  }


  hearts(uid:string,val:number){
  const id = this.routes.snapshot.params['num'];
  this.mechanicalSvc.hearts(id,uid,val);
  }
  ...
  ngOnInit() {
    this.mechanicals.subscribe((x) => {
        ...
        this.mechanical = Object.assign({},x);
...
parseCode(data:any):any{
  return data.replace(/\[code]([^]*?)\[\/code\]/gm,'<code>$1</code>').replace(/\`([^]*?)\`/gm ,'<code>$1</code>');
}
