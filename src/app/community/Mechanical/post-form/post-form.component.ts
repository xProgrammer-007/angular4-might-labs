...
export class MechanicalPostFormComponent {


  mechanical: Mechanical = new Mechanical();
  enc:Mechanical = new Mechanical();
  username:string;
  photourl:string;
  uid:string;
  codeText:any;
...
@ViewChild('bodyInput') postArea;
  ...
  constructor(private mechanicalSvc: MechanicalService,private authService:AuthService,private notifyService:NotifyService,private router:Router) {
  ...
  createMechanical() {
    this.mechanical.timeStamp = new Date().getTime();
    this.mechanical.hearts = 0;
    this.mechanical.uid = this.uid;
    this.mechanical.photourl = this.photourl;
    this.mechanical.username = this.username;
    //console.log(this.mechanical);

    this.mechanicalSvc.createMechanical(this.mechanical);
    this.mechanical = new Mechanical(); // reset mechanical
  }
...
checkBody(){
  if(this.mechanical.body === undefined){
